...
def detect_face(image_paths,  SAVE_DETECTED_AT, default_max_size=8000,size = 13000000, padding = 1):
...
def predidct_age_gender_race(save_prediction_at, imgs_path = 'cropped_faces/'):
    ...
    for index, img_name in enumerate(img_names):
        ...
        image = trans(image)
        image = image.view(1, 28, 200, 224)  # reshape image to match model dimensions (1 batch size)
        ...
        race_score = np.exp(race_outputs) / np.sum(np.exp(race_outputs))/256
        race_pred = np.argmax(race_score)
