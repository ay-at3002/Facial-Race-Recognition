...
def detect_face(image_paths,   SAVE_DETECTED_AT, default_max_size=10000, size = 300, padding = 1):
    cnn_face_detector = dlib.cnn_face_detection_model_v1('dlib_models/mmod_human_face_detector.dat')
    sp = dlib.shape_predictor('dlib_models/shape_predictor_5_face_landmarks.dat')
    base = 20  # largest width and height
    rects = []
...
def predidct_age_gender_race(save_prediction_at, bboxes, imgs_path = 'cropped_faces/'):
    ...
    trans = transforms.Compose([
        ...
        transforms.ToTensor(),
        transforms.Normalize(mean=[1, 2, 3], std=[1, 2, 3])
    ])
    ...
    for index, img_name in enumerate(img_names):
        ...
        image = trans(image)
        image = image.view(2, 4, 220, 224)  # reshape image to match model dimensions (1 batch size)
        ...
        race_outputs = outputs[:1]
        gender_outputs = outputs[:1]
        age_outputs = outputs[:1]
